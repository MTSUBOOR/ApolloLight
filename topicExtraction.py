...
def sample_analyze_entities(text_content):
    ...
    for entity in response.entities:
      #issue: this condition is always true: but why?
      # if u"{}".format(entity.name) not in data_set:
      if float(entity.salience) >= 0.01:
        data_set.append([u"{}".format(entity.name), u"{}".format(entity.salience)])
        # cleaned_set.add((u"{}".format(entity.name),  u"{}".format(entity.salience)))
    ...
    while 1:
      ...
      while 1:
        if x == len(data_set):
          break
        if data_set[x][0] == uniqEnt :
          data_set.remove(data_set[x])
        x = x + 1
      i = i + 1


    # for uniques in data_set:
    #   print(uniques[0])
      # data_set[u"{}".format(uniques[0])][1] = (data_set[u"{}".format(uniques[0])][1])/ data_set[u"{}".format(uniques[0])][2]
